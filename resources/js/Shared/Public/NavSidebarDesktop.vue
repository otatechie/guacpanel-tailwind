...
<template>
  <aside
    ...
    class="flex h-full flex-col bg-[var(--color-surface)] shadow-lg transition-all duration-300 ease-in-out">
          ...
          <template v-for="(item, itemIndex) in section.items" :key="itemIndex">
            ...
            <li v-else>
              <Link
                :href="item.href"
                :class="{
                  'flex items-center rounded-lg px-2.5 py-2 transition-all duration-200 ease-in-out': true,
                  'text-teal-600 dark:text-teal-400': isActive(item.href),
                  'text-[var(--color-text-muted)] hover:bg-[var(--color-surface-muted)] hover:text-[var(--color-text)]':
                    !isActive(item.href),
                }">
                <svg
                  class="mr-2.5 h-[18px] w-[18px] transition-colors duration-200"
                  :class="{
                    'text-teal-600 dark:text-teal-400': isActive(item.href),
