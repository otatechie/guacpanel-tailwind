...
const styles = {
  input:
    'border border-[var(--color-border-strong)] rounded-md text-sm bg-[var(--color-surface)] text-[var(--color-text)] focus:ring-2 focus:ring-opacity-50 focus:border-transparent',
...
const paginationInfo = computed(() => {
  ...
  const pageSize = isServerPagination.value ? props.pagination.per_page : pagination.value.pageSize
...
<template>
  <section class="relative">
    ...
    <div
      v-if="loading"
      role="status"
      class="absolute inset-0 z-10 flex items-center justify-center bg-[color:rgb(255_255_255_/_.5)] dark:bg-[color:rgb(17_24_39_/_.5)]">
      <span
    ...
    <header
      ...
      <div class="flex w-full flex-col items-start gap-3 sm:w-auto sm:flex-row sm:items-center">
        <div class="flex items-center space-x-2">
          ...
          <select
            class="focus:ring-opacity-50 rounded-md border border-[var(--color-border-strong)] bg-[var(--color-surface)] text-sm text-[var(--color-text)] focus:ring-2 focus:outline-none"
      ...
      <nav class="flex w-full flex-col items-start gap-3 sm:w-auto sm:flex-row sm:items-center">
        <div v-if="enableSearch" class="relative w-full sm:w-48">
          ...
          <button
            ...
            :class="[
              styles.focusRing,
              'absolute top-1/2 right-2 -translate-y-1/2 text-[var(--color-text-muted)] hover:text-[var(--color-text)]',
            ]"
            :style="{ '--tw-ring-color': 'var(--primary-color)' }"
    ...
    <div class="overflow-x-auto rounded-lg border border-[var(--color-border)]">
      <div class="block space-y-3 p-3 md:hidden">
        <div
          class="flex items-center justify-between rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-2">
          <label class="inline-flex items-center">
            <input
              type="checkbox"
              class="form-checkbox focus:ring-opacity-50 rounded border-[var(--color-border-strong)] bg-[var(--color-surface)] focus:ring-2"
              :style="{
        ...
        <div
          ...
          :key="row.id"
          class="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm transition-all duration-200 hover:shadow-md">
          <div class="p-2">
            <div class="mb-1.5 flex items-center justify-between">
              <div class="flex items-center gap-1.5">
                <label class="inline-flex items-center">
                  ...
                  <span class="ml-1.5 text-xs font-medium text-[var(--color-text)]">Select</span>
                </label>
                ...
                <button
                  ...
                  <svg
                    class="h-3 w-3 text-[var(--color-text-muted)] transition-transform duration-200 group-hover:text-[var(--color-text)]"
                    :class="{
            ...
            <div class="mb-1.5 border-b border-[var(--color-border)]"></div>
      ...
      <table class="hidden min-w-full divide-y divide-[var(--color-border)] md:table" role="grid">
            ...
            <th class="w-10 px-6 py-3">
              <div class="flex items-center">
                <label class="inline-flex items-center">
                  <input
                    type="checkbox"
                    class="form-checkbox focus:ring-opacity-50 rounded border-[var(--color-border-strong)] bg-[var(--color-surface)] text-blue-600 focus:ring-2"
                    :style="{
                      '--tw-ring-color': 'var(--primary-color)',
                      color: selectionColor,
                    }"
      ...
      <nav class="flex items-center gap-2" aria-label="Pagination">
        <template v-if="isServerPagination">
          ...
          <div class="flex items-center gap-1">
            <span class="hidden text-xs text-[var(--color-text-muted)] sm:inline">Page</span>
            <input
              type="number"
              :value="currentPage"
    ...
    <Modal :show="showDeleteModal" size="sm" @close="showDeleteModal = false">
      <template #title>
        <div class="flex items-center gap-2 text-red-600 dark:text-red-400">Confirm Deletion</div>
      </template>
      ...
      <template #footer>
        <div class="flex justify-end gap-8">
          <button
            type="button"
            class="cursor-pointer text-sm font-medium text-[var(--color-text)] hover:text-[var(--color-text-muted)]"
