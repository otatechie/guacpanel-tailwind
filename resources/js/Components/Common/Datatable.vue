...
<template>
  <section class="relative">
    <div v-if="error" role="alert" class="mb-4 rounded-md bg-red-50 p-4 text-red-700 dark:bg-red-950 dark:text-red-400">
      {{ error }}
    </div>

    <div v-if="loading" role="status"
      class="absolute inset-0 z-10 flex items-center justify-center bg-white/50 dark:bg-gray-900/50">
      <span class="h-8 w-8 animate-spin rounded-full border-b-2"
        :style="{ borderColor: 'var(--primary-color)' }"></span>
    </div>

    <header class="mb-4 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
      <div class="flex w-full flex-col items-start gap-3 sm:w-auto sm:flex-row sm:items-center">
        <div class="flex items-center space-x-2">
          ...
          <select
            ...
            :style="{ '--tw-ring-color': 'var(--primary-color)' }" :value="isServerPagination
              ? isAllSelected
                ? 'all'
                : String(props.pagination.per_page)
              : String(pagination.pageSize)
              " @change="handlePageSizeChange">
            <option v-for="size in pageSizeOptions" :key="size" :value="size === 'All' ? 'all' : size">
              {{ size === 'All' ? 'All' : size }}
            </option>
          </select>
        </div>
        ...
        <div v-if="hasSelection" class="flex items-center gap-6">
          <span role="status" class="flex items-center gap-1.5 text-xs font-medium text-gray-700 dark:text-gray-300">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="h-4 w-4 text-green-600 dark:text-green-500">
      ...
      <nav class="flex w-full flex-col items-start gap-3 sm:w-auto sm:flex-row sm:items-center">
        <div v-if="enableSearch" class="relative w-full sm:w-48">
          <label class="sr-only" :for="'table-search'">Search table</label>
          <input type="text" :value="globalFilter ?? ''" @input="e => (globalFilter = String(e.target.value))"
            :class="[styles.input, styles.focusRing, 'w-full px-4 py-2 pr-8'].join(' ')"
            :style="{ '--tw-ring-color': 'var(--primary-color)' }" placeholder="Search all columns..." />
          <button v-if="globalFilter" @click="globalFilter = ''" :class="[
            styles.focusRing,
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              <div class="flex items-center gap-1.5">
                ...
                <button @click="toggleRow(index)"
                  ...
                  <svg
                    ...
                    :class="{ 'rotate-90': expandedRows.includes(index) }" xmlns="http://www.w3.org/2000/svg"
      ...
      <table class="hidden min-w-full divide-y divide-gray-200 md:table dark:divide-gray-700" role="grid">
        <thead class="bg-gray-50 dark:bg-gray-900">
          ...
          ">
            ...
            <th v-for="header in table.getHeaderGroups()[0].headers" :key="`filter-${header.id}`" class="px-6 py-2">
              <Filter v-if="header.column.getCanFilter()" :column="header.column" :table="table" />
            </th>
          </tr>
        </thead>
    ...
    <footer class="mt-6 flex flex-col items-center justify-between gap-4 px-3 sm:flex-row sm:px-1">
      ...
      <nav class="flex items-center gap-2" aria-label="Pagination">
        <template v-if="isServerPagination">
          <button :class="[styles.button, styles.focusRing]" :style="{ '--tw-ring-color': 'var(--primary-color)' }"
            :disabled="isFirstPage" @click="goToPage(1)">
            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" v-html="icons.firstPage"></svg>
          </button>

          <button :class="[styles.button, styles.focusRing]" :style="{ '--tw-ring-color': 'var(--primary-color)' }"
            :disabled="isFirstPage" @click="goToPage(paginationInfo.currentPage - 1)">
            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" v-html="icons.prevPage"></svg>
          </button>
          ...
          <div class="flex items-center gap-1">
            ...
            <input type="number" :value="paginationInfo.currentPage" @change="handlePageChange"
          ...
          <button :class="[styles.button, styles.focusRing]" :style="{ '--tw-ring-color': 'var(--primary-color)' }"
            :disabled="isLastPage" @click="goToPage(paginationInfo.currentPage + 1)">
            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" v-html="icons.nextPage"></svg>
          </button>

          <button :class="[styles.button, styles.focusRing]" :style="{ '--tw-ring-color': 'var(--primary-color)' }"
            :disabled="isLastPage" @click="goToPage(paginationInfo.pageCount)">
            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" v-html="icons.lastPage"></svg>
          </button>
        </template>

        <template v-else>
          <button class="rounded-md px-2 py-1 disabled:cursor-not-allowed disabled:opacity-50" :class="[
            table.getCanPreviousPage()
              ? 'text-gray-700 hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-gray-700'
              : 'text-gray-400 dark:text-gray-600',
            styles.focusRing,
          ]" :style="table.getCanPreviousPage()
            ? { '--tw-ring-color': 'var(--primary-color)' }
            : {}
    ...
    <Modal :show="showDeleteModal" @close="showDeleteModal = false" size="sm">
      ...
      <template #footer>
        <div class="flex justify-end gap-8">
          ...
          <button type="button" :disabled="loading" @click="handleBulkDelete" class="btn-3d-danger btn-sm">
            <template v-if="loading">
              <svg class="mr-2 -ml-1 h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none"
                viewBox="0 0 24 24">
