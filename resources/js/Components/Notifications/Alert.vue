...
<template>
  <div v-if="isVisible" :class="[
    'rounded-lg border transition-all duration-150 ease-in-out',
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-shrink-0">
        <svg :class="[sizeConfig.icon, alertConfig.iconColor]" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div class="min-w-0 flex-1">
        <h3 v-if="title" :class="[sizeConfig.title, 'mb-1 font-medium', alertConfig.text]">
          {{ title }}
        </h3>
      ...
      <button v-if="dismissible"
        class="ml-2 flex-shrink-0 cursor-pointer rounded-md p-1 transition-colors hover:bg-black/5 dark:hover:bg-white/5"
        :class="alertConfig.text" type="button" aria-label="Dismiss alert" @click="handleDismiss">
