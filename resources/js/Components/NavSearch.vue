    ...
    <div class="relative">
        ...
        <Transition enter-active-class="transition duration-100 ease-out"
            ...
            <form :class="{ 'hidden md:block': !isOpen }" @submit.prevent="handleSearch"
                ...
                role="search">
                <label class="relative w-60 md:w-80 lg:w-96">
                    <!-- Search Icon -->
                    ...
                    <input v-model="query" type="search" placeholder="Search phones, laptops..."
                        ...
                        :aria-expanded="query.trim() && (searchResults.length > 0 || !isLoading)"
                ...
                <div v-if="query.trim() && (searchResults.length > 0 || !isLoading)"
                    ...
                    <div v-if="!isLoading && searchResults.length === 0"
                        ...
                        role="status">
                        <span class="w-24 h-24 mx-auto mb-4 flex items-center justify-center text-gray-400">
                            <i class="pi pi-search text-4xl" aria-hidden="true"></i>
                        </span>
                    ...
                    <ul v-else>
                        <li v-for="result in searchResults"
                            ...
                            <div class="flex items-center gap-4">
                                <figure class="w-16 h-16 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
                                    <img v-if="result.images?.[0]"
                                        :src="result.images[0]"
                                        :alt="`${result.phone_brand?.name} ${result.phone_variant?.name}`"
                                        class="w-full h-full object-cover"
                                        @error="handleImageError">
                                    <span v-else
                                        class="w-full h-full flex items-center justify-center text-gray-400">
                                        <i class="pi pi-mobile text-xl" aria-hidden="true"></i>
                                    </span>
                                </figure>
