...
const isParentActive = children =>
    children?.some?.(child => isActive(child.route)) ?? false
        ...
        items: [
            {
                ...
                children: [
                    ...
                    { name: 'Users', route: 'admin.user.index', permission: 'manage-users' },
                    { name: 'Audit Logs', route: 'admin.audit.index', permission: 'view-audits' },
                    { name: 'Security Settings', route: 'admin.setting.show', permission: 'manage-security-settings' },
                ]
            },
            { type: 'divider' },
        ]
                    ...
                    <template v-if="sectionHasVisibleItems(section)" v-for="(item, itemIndex) in section.items"
                            ...
                            <ul v-show="item.isOpen" class="pl-9 space-y-1">
                                <li v-for="child in getVisibleChildren(item.children)" :key="child.name">
                                    <Link :href="route(child.route)" :class="[
                                        'nav-child-item',
                                        isActive(child.route) ? 'nav-child-item-active' : 'nav-child-item-default'
                                    ]">
                        ...
                        <li v-else>
                            <Link v-if="hasPermission(item.permission)" :href="route(item.route)" :class="[
                                'nav-item',
                                isActive(item.route) ? 'nav-item-active' : 'nav-item-default'
                            ]">
                            <svg :class="[
                                'nav-icon',
                                isActive(item.route) ? 'nav-icon-active' : 'nav-icon-default'
