      ...
      <PageHeader
        ...
        <template #actions>
          <button
            ...
            <svg v-if="isRunning" class="mr-2 h-3 w-3 animate-spin" fill="none" viewBox="0 0 24 24">
              <circle
                class="opacity-25"
                cx="12"
                cy="12"
      ...
      <section class="p-6 dark:bg-gray-900">
        <div
          ...
          <div
            ...
            class="mb-6 flex flex-col gap-3 border-b border-gray-100 pb-4 sm:flex-row sm:items-center sm:justify-between dark:border-gray-700">
            ...
            <div class="flex flex-wrap items-center gap-2">
              <span
                class="rounded-md bg-green-50 px-2 py-0.5 text-[11px] font-medium text-green-700 dark:bg-green-900/30 dark:text-green-300">
                OK: {{ counts.ok }}
              </span>
              <span
                class="rounded-md bg-yellow-50 px-2 py-0.5 text-[11px] font-medium text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300">
                Warn: {{ counts.warning }}
              </span>
              <span
                class="rounded-md bg-red-50 px-2 py-0.5 text-[11px] font-medium text-red-700 dark:bg-red-900/30 dark:text-red-300">
                Fail: {{ counts.failed }}
              </span>
              <span
                class="rounded-md bg-red-100 px-2 py-0.5 text-[11px] font-medium text-red-700 dark:bg-red-900/50 dark:text-red-300">
                Crash: {{ counts.crashed }}
              </span>
              <span
                class="rounded-md bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                Total: {{ counts.total }}
              </span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 lg:gap-6">
            <template v-if="isRunning">
              <div
                v-for="n in 6"
                :key="n"
                class="animate-pulse rounded-lg border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div class="flex items-start gap-3">
                  <div class="h-8 w-8 rounded-full bg-gray-100 dark:bg-gray-700"></div>
                  <div class="flex-1 space-y-3">
                    <div class="h-4 w-1/3 rounded bg-gray-100 dark:bg-gray-700"></div>
                    <div class="h-3 w-2/3 rounded bg-gray-100 dark:bg-gray-700"></div>
                  </div>
                </div>
              </div>
            </template>
            <div
              ...
              class="col-span-full flex flex-col items-center justify-center px-6 py-12 text-center">
              <div
            ...
            <template v-for="(group, status) in groupedResults" v-else :key="status">
              <div
                ...
                :key="result.label"
                class="rounded-lg border border-gray-100 bg-white p-5 shadow-sm transition-all duration-200 hover:shadow-md dark:border-gray-700 dark:bg-gray-800">
                <div class="flex items-start gap-3">
                  <div
                    :class="[
                  ...
                  <div class="flex-1">
                    ...
                    <div
                      ...
                      class="mt-3 border-t border-gray-100 pt-3 text-xs dark:border-gray-700">
