...
<template>
    <div class="max-w-2xl space-y-5">
        <div v-if="deactivateEnabled">
            <p class="text-base font-medium text-foreground">Deactivate account</p>
            <p class="mt-1 text-sm text-muted-foreground">Temporarily hide your profile and data. You can reactivate anytime.</p>
        ...
        <div :class="deactivateEnabled ? 'border-t border-red-200 pt-5 dark:border-red-900/30' : ''">
            ...
            <p class="mt-1 text-sm text-muted-foreground">Permanently delete your account and all data. This cannot be undone.</p>
            <Button variant="danger" size="sm" class="mt-3" v-if="deleteEnabled" @click="deleteModal = true">Delete account</Button>
            <p v-else class="mt-2 text-xs text-muted-foreground">Account deletion is disabled.</p>
        </div>
    </div>
    ...
    <Modal :show="deactivateModal" @close="deactivateModal = false" size="sm">
        ...
        <template #default>
            <p class="text-sm text-muted-foreground">Your account will be hidden but can be reactivated later.</p>
        </template>
        <template #footer>
    ...
    <Modal :show="deleteModal" @close="deleteModal = false" size="sm">
        ...
        <template #default>
            <p class="text-sm text-muted-foreground">This permanently deletes your account and all associated data.</p>
        </template>
        <template #footer>
            <div class="flex justify-end gap-3">
