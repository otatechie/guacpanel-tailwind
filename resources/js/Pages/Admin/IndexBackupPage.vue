        ...
        <section class="container-border overflow-hidden">
            <div class="bg-gray-50 px-6 py-3 border-b border-gray-200">
                <div class="flex items-center space-x-2 text-sm">
                    ...
                    <span class="text-gray-800">Backup Management</span>
                </div>
            </div>
            ...
            <header class="px-6 py-5 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="sub-heading">Backup Management</h1>
                        <p class="mt-1 text-gray-500">
                            Manage system backups and storage locations
                        </p>
                    </div>
                    <button
                        type="button"
                    ...
                    >
                        <svg v-if="isBackupRunning" class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-6">
                ...
                <div v-for="info in backupInfo" :key="info.name" class="space-y-6">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <article v-for="(stat, index) in getStats(info)" :key="index" class="bg-white rounded-lg border border-gray-200 p-4">
                            <div class="flex items-center gap-3 mb-1">
                                ...
                                <h2 class="text-sm font-medium text-gray-600">{{ stat.title }}</h2>
                            </div>
                            <p :class="[
                                stat.type === 'status' ?
                    ...
                    <div v-if="info.backups?.length > 0" class="bg-white border border-gray-200 rounded-lg overflow-hidden">
                        <div class="overflow-x-auto">
                            <table class="w-full divide-y divide-gray-200 md:table hidden">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    <tr v-for="backup in info.backups" :key="backup.path" class="hover:bg-gray-50 transition-colors">
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-800">{{ backup.date }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-800">{{ backup.size }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 font-mono truncate max-w-[200px]">{{ backup.path }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-right space-x-1">
                                            <button type="button" @click="downloadBackup(backup.path)" class="inline-flex items-center p-2 text-gray-600 hover:text-blue-600 rounded-md hover:bg-blue-50 transition-colors cursor-pointer" title="Download backup" :disabled="!backup.path">
                                                <span class="sr-only">Download backup</span>
                                                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                                                </svg>
                                            </button>
                                            <button type="button" @click="deleteBackup(backup.path)" class="inline-flex items-center p-2 text-gray-600 hover:text-red-600 rounded-md hover:bg-red-50 transition-colors cursor-pointer" title="Delete backup" :disabled="!backup.path">
                        ...
                        <div class="md:hidden divide-y divide-gray-200">
                            <div v-for="backup in info.backups" :key="backup.path" class="p-4 space-y-2 hover:bg-gray-50 transition-colors">
                                <div class="flex justify-between items-start">
                                    <div>
                                        <p class="text-sm font-medium text-gray-800">{{ backup.date }}</p>
                                        <p class="text-sm text-gray-500">{{ backup.size }}</p>
                                    </div>
                                    <div class="flex space-x-1">
                                        <button @click="downloadBackup(backup.path)" class="p-2 text-gray-600 hover:text-blue-600 rounded-md hover:bg-blue-50 transition-colors" title="Download backup">
                                            <i class="pi pi-download text-lg"></i>
                                        </button>
                                        <button @click="deleteBackup(backup.path)" class="p-2 text-gray-600 hover:text-red-600 rounded-md hover:bg-red-50 transition-colors" title="Delete backup">
                    ...
                    <div v-else class="text-center p-8 bg-white rounded-xl border border-gray-200">
