...
const actionBtnClass = 'cursor-pointer rounded-md p-1.5 text-[var(--color-text-muted)] transition-colors hover:bg-[var(--color-surface-muted)] hover:text-[var(--color-text)]'
</script>
...
<template>
    ...
    <main class="mx-auto max-w-7xl" aria-labelledby="system-backups">
        ...
        <div class="space-y-4">
            <Alert type="info">
                ...
                <code class="ml-1 rounded bg-(--color-surface-muted) px-1.5 py-0.5 font-mono text-xs">php artisan backup:run</code>
            </Alert>

            <section v-for="info in backupInfo" :key="info.name">
                <!-- Summary -->
                <div class="mb-3 flex flex-wrap items-center gap-x-5 gap-y-1 text-xs text-(--color-text-muted)">
                    <span>Disk: <span class="font-medium text-(--color-text)">{{ info.disk }}</span></span>
                    <span>Storage: <span class="font-medium text-(--color-text)">{{ info.storageType }}</span></span>
                    <span>Used: <span class="font-medium text-(--color-text)">{{ info.storageSpace }}</span></span>
                ...
                <!-- Backups list -->
                <div v-if="info.backups?.length > 0" class="card divide-y divide-(--card-border) overflow-hidden">
                    <div
                        v-for="backup in info.backups"
                        :key="backup.path"
                        class="flex items-center justify-between gap-4 px-4 py-3 transition-colors hover:bg-(--color-surface-muted)">
                        <div class="min-w-0 flex-1">
                            <div class="flex items-baseline gap-3">
                                <span class="text-sm font-medium text-(--color-text)">{{ backup.date }}</span>
                                <span class="text-xs tabular-nums text-(--color-text-muted)">{{ backup.size }}</span>
                            </div>
                            <p class="mt-0.5 truncate font-mono text-[11px] text-(--color-text-muted)">{{ backup.path }}</p>
                        </div>
                ...
                <p v-else class="card px-5 py-8 text-center text-sm text-(--color-text-muted)">
                    No backups found
                </p>
            </section>
        </div>
    ...
    <Modal :show="showDeleteModal" size="md" @close="closeDeleteModal">
        ...
        <template #default>
            ...
            <p v-if="selectedBackup" class="mt-2 truncate rounded-md bg-(--color-surface-muted) px-3 py-2 font-mono text-xs text-(--color-text-muted)">
                {{ selectedBackup.path }}
            </p>
        </template>
        <template #footer>
